...
DATA_PATH = ENGINE_DIR / "data" / "candidate.json"
TEMPLATE_PATH = ENGINE_DIR / "templates" / "resume.tex"
COVER_LETTER_TEMPLATE_PATH = ENGINE_DIR / "templates" / "cover_letter.tex"
RESUMES_DIR = ENGINE_DIR / "resumes"
...
def is_duplicate(company: str, role: str, url: str) -> bool:
    """Check if a job already exists via db.is_duplicate()."""
    return db.is_duplicate(company, role, url)
...
def process_job(
    ...
    9. Ingestion into Supabase jobs table.
    """
    ...
    # New fields
    work_setups = job_dict.get("work_setups", [])
    employment_type = job_dict.get("employment_type", "")
    min_salary_usd = job_dict.get("min_salary_usd")
    max_salary_usd = job_dict.get("max_salary_usd")
    ...
    if is_duplicate(company, role, url):
        return None
    ...
    job_data = {
        ...
        "matched_skills": ", ".join(matched_skills) if matched_skills else "Full-Stack Development",
        "hiring_contact": hiring_contact,
...
if __name__ == "__main__":
    ...
    total_jobs = len(jobs)
    to_review = sum(1 for j in jobs if j.get("status") == "To Review")
